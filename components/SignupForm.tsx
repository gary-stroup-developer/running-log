...
const SignupForm = () => {
  return (
      <div className='flex flex-col justify-center items-center w-full h-screen bg-gradient-to-r from-modernPrimary to-cyan-600'>
          ...
          >
              {({ isSubmitting }) => (
                  <Form className="flex flex-col justify-evenly font-poppins p-4 h-[85vh] rounded-md w-2/3 dark:bg-slate-800">
                      ...
                      <Button className='w-1/3 mx-auto text-md font-semibold bg-modernPrimary hover:bg-modernSecondary' type='submit' disabled={isSubmitting} size={'lg'}> <UserCheck2Icon className='mr-2 h-4 w-4' /> Submit</Button>
                  </Form>
              )}
          </Formik>
          <Link href={"/sign-in"} className='text-white text-lg p-3 rounded-lg hover:border hover:scale-125'>Already a member?</Link>
      </div>
  )
}
